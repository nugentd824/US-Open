...
export const mockScoreProvider = {
  ...
  async getScores(tournamentId) {
    ...
    const rows = field.map((g) => {
      const talent = talentForRank(g.oddsRank, fieldSize);
      // Small per-golfer tee-time jitter so "thru" varies across the field.
      const jitter = hashString(g.golferId) % 3;
      let holes = Math.max(0, Math.min(TOTAL_HOLES, totalHoles - jitter));
      let status = 'active';

      // --- Withdrawal / Disqualification (freeze at the hole they stopped) ---
      if (g.oddsRank === WD_RANK && holes >= WD_HOLE) {
        holes = WD_HOLE;
        status = 'wd';
      } else if (g.oddsRank === DQ_RANK && holes >= DQ_HOLE) {
        holes = DQ_HOLE;
        status = 'dq';
      }
      ...
      // Not yet teed off.
      if (holes === 0) {
        return mkRow(g, 0, 'not_started', 1, null);
      }
      ...
      return mkRow(g, toParAtHole(g.golferId, talent, holes), 'active', round, thru, talent);
    });
    ...
    if (totalHoles >= CUT_HOLE) {
      const cutEligible = rows.filter((r) => r.status === 'active' || r.status === 'cut');
      const scoredAt36 = cutEligible.map((r) => {
        const g = field.find((f) => f.golferId === r.golferId);
        const talent = talentForRank(g.oddsRank, fieldSize);
